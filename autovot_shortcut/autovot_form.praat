...
# === Changes (Yannick Jadoul) ===
# === * Converted later `beginPause` input into form parameters
form AutoVOT
	comment This is a script for automatic measurement of voice onset time (VOT)
	comment Sound & TextGrid parameters:
	sentence Interval_tier ""
	word Interval_mark "* (= any string)"
   	optionmenu Selected_channel: 1
		option mono
		option left
		option right
	comment Algorithm parameters:
	natural min_vot_length 5
	natural max_vot_length 500
	comment Training model files (relative to the above directory):
	text vot_classifier_model ""
endform
...
# === Changes (Yannick Jadoul) ===
# === * Commented out `beginPause` section, as these are added with a `form` such that the values can be passes as parameters to the script
# beginPause: "AutoVOT"
# 	comment: "This is a script for automatic measurement of voice onset time (VOT)"
# 	comment: "Sound & TextGrid parameters:"
